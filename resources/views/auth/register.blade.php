<x-guest-layout>
    <x-auth-card>
        ...
        <form method="POST" action="{{ route('register') }}" enctype="multipart/form-data">
            @csrf
            @if (!!request()->referral && ($user = \App\Models\User::find(request()->referral)))
                <!-- Referral -->
                <div>
                    <x-input-label for="name" :value="__('Referral')" />

                    you are registering via link from : {{ $user->name ?? '' }}

                    <x-input-error :messages="$errors->get('name')" class="mt-2" />
                </div>
                <input type="hidden" name="referral_id" value="{{ request()->referral ?? null }}">
            @endif
            <!-- Name -->
            <div>
